...
* **Task type:** Searching mit Untergruppenversprechen, wie bei Montanaro. Zurück kommt die Stabilizergruppe, also der Träger des Weyl-Spektrums, danach die Phasen $\omega^{s_i}$ der Generatoren. Die Zeile "Stabilizer states on qudits, $d > 2$" der Searching-Tabelle. Der Pseudozufallsteil ist ein Ein-Bit-Test und gehört zur Zeile "Clifford plus few non-Clifford gates" der Identifying-Tabelle.
...
* Die Involution $J$ ist, bis auf die Vorzeichenkonvention, dieselbe Spiegelung, die im Projekt als $D_{q,p}^* = D_{q,-p}$ auftaucht (Abschnitt "Consequence for the instance generator").
...
* Welche Stabilizerzustände sind reell in der Rechenbasis, sodass $\rho\otimes\rho$ schon $\rho\otimes\rho^*$ ist und Theorem 40 ohne Konjugation greift (Abschnitt "The conjugate: real with respect to a basis")?
...
* **Task type:** gemischt. Theorem 1 ist Estimating: der Betrag $\vert\mathrm{Tr}(O\rho)\vert$ einer Pauli-Observable, die erst nach der Messung genannt wird. Theorem 2 schätzt eine Eigenschaft der Hauptkomponente, Theorem 3 die Ausgaben eines Prozesses im Mittel. Das zweite Experiment ist Identifying: die Symmetrieklasse eines Unitaries. Das Paper steht bei Identifying neben Chen, Cotler, Huang, Li, weil alle unteren Schranken über Unterscheidungsaufgaben laufen. Für die Estimating-Zeile "All $4^n$ Pauli observables, two-copy" liefert es den Hardware-Beleg der Zwei-Kopien-Trennung, für die Zeile "Fixed-unitary and symmetry-class distinction" den der Kohärenz-Trennung bei Prozessen.
...
* Die erste Hardware-Demonstration einer beweisbar exponentiellen Speichertrennung beim Lernen aus Quantendaten. Auf sie stützen sich der Abschnitt "Bell sampling on two copies" und die Separation-These am Anfang dieses Dokuments.
...
* In Experiment 2 bedeutet Zeitumkehrsymmetrie reelle Amplituden. Das ist dieselbe Bedingung, unter der im Projekt das Konjugat gratis ist (Abschnitt "The conjugate: real with respect to a basis").
